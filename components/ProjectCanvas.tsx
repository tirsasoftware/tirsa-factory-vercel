...
import { SkipForward, RefreshCw, Pause, Play, FolderOpen, FileText, CheckCircle2, Zap } from "lucide-react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import { supabase } from "@/lib/supabase";
import type { AgentRun, AgentEvent, SquadName } from "@/lib/types";
...
function ArtifactTreeModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      .then(async (r) => {
        if (!r || !r.ok) return;
        const data = await r.json() as { storageFiles: { path: string; size: number | null }[] };
        setFiles(data.storageFiles ?? []);
      })
...
function PipelineSegment({
  ...
  const canStop        = (projectStatus === "executing" || projectStatus === "running") && anyAgentActive;
  const canMarkComplete = (projectStatus === "executing" || projectStatus === "running") && !anyAgentActive;
  const canContinue    = RESUMABLE_STATUSES.includes(projectStatus) || projectStatus === "ready" || projectStatus === "provisioning";
  ...
  // When the project hasn't started yet (ready/provisioning) don't highlight any agent.
  const notStarted = projectStatus === "ready" || projectStatus === "provisioning";
...
const ACTIVE_STATUSES   = ["executing", "running", "waiting", "provisioning"];
const RESUMABLE_STATUSES = ["paused", "waiting", "queued"];
...
export default function ProjectCanvas({
  ...
  const statusColor =
    projectStatus === "executing" ? "var(--green)" :
    projectStatus === "completed" ? "var(--blue)" :
    projectStatus === "paused" ? "var(--yellow)" :
    projectStatus === "waiting" ? "var(--peach)" :
    "var(--overlay0)";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {(projectStatus === "ready" || projectStatus === "provisioning") && (
