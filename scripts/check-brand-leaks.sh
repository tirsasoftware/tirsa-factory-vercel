...
# Invariant: lib/brand.active.ts must be committed with BRAND_ID=tirsa defaults.
# The source repo is brand-neutral — per-brand deploys regenerate this file in
# CI (sync-vercel.yml matrix). Committing a twinpilot-flipped version would
# leak into Tirsa's Vercel build before the matrix job runs.
if ! grep -q '"id": "tirsa"' lib/brand.active.ts 2>/dev/null; then
  echo "::error::lib/brand.active.ts is not the tirsa default."
  echo
  echo "The source repo commits brand.active.ts with BRAND_ID=tirsa. You probably"
  echo "flipped BRAND_ID locally to validate another brand — regenerate with:"
  echo
  echo "    cd services/command-center"
  echo "    npm run brand:prepare     # without BRAND_ID, uses tirsa default"
  echo
  echo "Then commit again. Per-brand builds happen in CI, not from committed state."
  exit 1
fi

echo "✓ brand guardrail: brand.active.ts is tirsa default"
