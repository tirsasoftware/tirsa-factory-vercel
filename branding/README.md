...
1. `scripts/brand-prebuild.mjs` lê `BRAND_ID` (default `tirsa`).
...
O prebuild roda automaticamente em `npm install`, `npm run dev` e `npm run build` via hooks do `package.json`. Pra flipar localmente pra validar outra marca:

```bash
BRAND_ID=twinpilot npm run dev            # dev sob TwinPilot
BRAND_ID=twinpilot npm run build          # build prod sob TwinPilot
BRAND_ID=twinpilot npm run brand:prepare  # só regerar artefatos
```

Antes de commitar, volte pra tirsa default:

```bash
npm run brand:prepare     # sem BRAND_ID, usa tirsa
```

**Regra:** `lib/brand.active.ts` vai commitado com `BRAND_ID=tirsa` default. O source repo é brand-neutral; quem gera deploys per-brand é o `sync-vercel.yml` matrix (veja `docs/WHITE-LABEL.md`). O guardrail CI (`scripts/check-brand-leaks.sh`) reforça essa invariante — falha se detectar `brand.active.ts` não-tirsa.
