...
const USER_SPACE_TOOLS = [
  { name: "write_staging_artifact", desc: "Write sprint deliverables (docs, code) to the staging area" },
  { name: "write_project_file",    desc: "Write committed project files (src/, docs/, specs/, etc.)" },
  { name: "read_project_file",     desc: "Read files from the project workspace" },
  { name: "list_project_files",    desc: "List files in the project workspace" },
  { name: "read_artifact",         desc: "Read outputs from previous pipeline agents" },
  { name: "list_artifacts",        desc: "List available staging artifacts" },
] as const;

const KNOWLEDGE_TOOLS = [
  { name: "search_knowledge",      desc: "Semantic search across linked knowledge instances" },
  { name: "list_knowledge_sources", desc: "List available knowledge sources" },
  { name: "fetch_url",             desc: "Fetch and parse URL content (cached per sprint)" },
] as const;

const PIPELINE_TOOLS = [
  { name: "escalate_to_human",    desc: "Pause pipeline and request human input" },
  { name: "github_push_sprint",   desc: "Commit staged files to GitHub, create branch, open PR, and optionally tag the sprint" },
  { name: "mark_sprint_saved",    desc: "Mark the current sprint as persisted/saved after committing artifacts externally" },
] as const;
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
            <Terminal size={22} color="var(--blue)" />
            <h1 style={{ fontSize: 22, fontWeight: 800, margin: 0, fontFamily: "var(--font-heading)" }}>Tools</h1>
          </div>
          <p style={{ fontSize: 13, color: "var(--subtext0)", margin: "0 0 24px", lineHeight: 1.6 }}>
            Tool catalog and MCP server configuration for AI agents.
          </p>
            ...
            <div>
              ...
              <div style={cardStyle}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 99, background: "rgba(166,227,161,0.12)", color: "var(--green)", textTransform: "uppercase" }}>Local only</span>
                </div>
                <p style={{ fontSize: 12, color: "var(--subtext0)", margin: "0 0 14px", lineHeight: 1.6 }}>
                  File operations on the user&apos;s local filesystem. Available via MCP to CLI agents running in <strong style={{ color: "var(--text)" }}>Local mode</strong>.
                </p>
              ...
              <div style={cardStyle}>
                <ul style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 10 }}>
                  <li style={{ fontSize: 13, color: "var(--subtext1)", lineHeight: 1.6 }}>MCP tools are available in <strong style={{ color: "var(--text)" }}>Local mode</strong>. In Cloud mode, agents use API calls directly.</li>
                  <li style={{ fontSize: 13, color: "var(--subtext1)", lineHeight: 1.6 }}>The MCP server runs as a subprocess alongside the CLI. Config is written to <code style={codeStyle}>.mcp.json</code>.</li>
                  <li style={{ fontSize: 13, color: "var(--subtext1)", lineHeight: 1.6 }}>MCP-capable CLIs: <strong style={{ color: "var(--text)" }}>Claude Code</strong>, <strong style={{ color: "var(--text)" }}>Goose</strong>, <strong style={{ color: "var(--text)" }}>Plandex</strong></li>
                  <li style={{ fontSize: 13, color: "var(--subtext1)", lineHeight: 1.6 }}>Non-MCP CLIs (<strong style={{ color: "var(--text)" }}>Aider</strong>, <strong style={{ color: "var(--text)" }}>Codex</strong>) receive context via pre-loaded files.</li>
                </ul>
              </div>
            </div>
          )}
