...
import {
  Plus, Lock, Play, Zap, GitBranch, Shield, ChevronRight, ChevronDown,
  X, Sparkles, AlertTriangle, Trash2, Bot, ToggleLeft, ToggleRight,
...
function PipelineBuilderModal({
  ...
  const [allAgents, setAllAgents] = useState<{ slug: string; name: string; squad: string | null; level: string | null; parent_slug: string | null; spec: Record<string, unknown> | null }[]>([]);

  useEffect(() => {
    if (!tenantId) return;
    const q = supabase.from("agent_definitions").select("slug, name, squad, level, parent_slug, tenant_id, origin, spec").eq("enabled", true).eq("tenant_id", tenantId);
    if (factoryId) q.eq("factory_id", factoryId);
    q
      .then(({ data }) => {
        if (data) {
          const typed = data as { slug: string; name: string; squad: string | null; level: string | null; parent_slug: string | null; tenant_id: string | null; origin: string; spec: Record<string, unknown> | null }[];
          setAllAgents(typed);
        }
      });
  }, [tenantId]);
  ...
  async function handleSave() {
    ...
    if (isEdit && pipeline) {
      res = await fetch(`/api/pipelines/${pipeline.id}`, {
        ...
        body: JSON.stringify({ name, description, steps: cleanSteps, mode: pipelineMode }),
      });
    } else {
      res = await fetch("/api/pipelines", {
        method: "POST",
        headers: { Authorization: `Bearer ${authSession.access_token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ tenantId, slug: slugVal, name, description, steps: cleanSteps, factoryId, mode: pipelineMode }),
      });
    }
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--mantle)" }}>
        ...
        <div style={{ padding: "10px 20px", borderBottom: "1px solid var(--surface0)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input value={name} onChange={(e) => { const v = e.target.value; setName(v); setError(null); if (!slugTouched) setSlugVal(v.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")); }} placeholder="Pipeline name" style={{ ...inputStyle, fontWeight: 700, fontSize: 14, flex: 1, padding: "5px 10px" }} />
            {!isEdit && (
              <input value={slugVal} onChange={(e) => { setSlugTouched(true); setSlugVal(e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")); }} placeholder="slug" style={{ ...inputStyle, fontFamily: "var(--font-mono)", width: 120, fontSize: 11, padding: "5px 8px" }} />
            )}
          ...
          {selected && !(hideBuiltIn && selected.type === "system") ? (
            <>
              <div>
                ...
                {selected.description && <div style={{ fontSize: 11, color: "var(--subtext0)", marginTop: 1 }}>{selected.description}</div>}
              </div>
              {stats && (
...
function AgentsTab({ userRole, hideBuiltIn, tenantId, factoryName, factoryOrigin, factoryId, inheritedFactoryIds }: { userRole: UserRole; hideBuiltIn: boolean; tenantId: string | null; factoryName: string | null; factoryOrigin: string; factoryId: string | null; inheritedFactoryIds: string[] }) {
  ...
  const [newForm,        setNewForm]        = useState({ name: "", slug: "", squad: "", autonomy: "auto" as AgentAutonomy, level: "" as AgentLevel | "", parent_slug: "", instructions: "", icon: "" });
  ...
  useEffect(() => {
    ...
    supabase.from("agent_definitions")
      .select("id, slug, name, level, enabled, origin, parent_slug, metadata, tenant_id, factory_id, squad, spec, version, icon, tags")
      .eq("tenant_id", tenantId)
      .order("name")
      .then(({ data, error }) => { if (!error && data) setAgents(data as unknown as AgentRow[]); setLoading(false); });
  ...
  function resetForm() {
    setNewForm({ name: "", slug: "", squad: "", autonomy: "auto", level: "", parent_slug: "", instructions: "", icon: "" });
  ...
  function openEditModal(agent: AgentRow) {
    ...
    setNewForm({
      ...
      squad: agent.squad ?? "",
      autonomy: ((agent.spec as Record<string, unknown>)?.autonomy as string ?? "auto") as AgentAutonomy,
      level: agent.level ?? "",
      parent_slug: agent.parent_slug ?? "",
      instructions: String(spec.description ?? spec.process ?? spec.freestyle_process ?? meta.instructions ?? ""),
  ...
  async function saveAgent() {
    ...
    // Build metadata
    let metadata: Record<string, unknown> = { ...(editAgent?.metadata ?? {}) };

    if (newForm.level === "super-specialist" && !newForm.parent_slug) {
      setFormError("Super-specialist agents must have a parent specialist selected.");
      return;
    }
    // Auto-populate sub_specialists for specialist agents
    const subs = agents
      .filter((a) => a.parent_slug === newForm.slug && a.level === "super-specialist")
      .map((a) => ({ slug: a.slug, name: a.name }));
    if (subs.length > 0) {
      metadata = { ...metadata, sub_specialists: subs };
    }

    // Build spec JSONB from form
    const specPayload: Record<string, unknown> = {
      description: newForm.instructions || "",
      output_types: (editAgent?.spec as Record<string, unknown>)?.output_types ?? [],
      suggested_inputs: (editAgent?.spec as Record<string, unknown>)?.suggested_inputs ?? [],
      tools: selectedTools,
      autonomy: newForm.autonomy,
      human_gate_reason: (editAgent?.spec as Record<string, unknown>)?.human_gate_reason ?? "",
      sla: (editAgent?.spec as Record<string, unknown>)?.sla ?? "",
      guardrails: (editAgent?.spec as Record<string, unknown>)?.guardrails ?? "",
      accept_external_instructions: (editAgent?.spec as Record<string, unknown>)?.accept_external_instructions ?? true,
      model_preference: (editAgent?.spec as Record<string, unknown>)?.model_preference ?? "",
      max_rounds: (editAgent?.spec as Record<string, unknown>)?.max_rounds ?? 0,
    };
    ...
    const payload = {
      ...
      parent_slug: newForm.level === "super-specialist" ? (newForm.parent_slug || null) : null,
      metadata,
      spec: specPayload,
    };
    if (editAgent) {
      ...
      const { data, error } = await supabase.from("agent_definitions")
        ...
        .select("id, slug, name, level, enabled, origin, parent_slug, metadata, squad, spec, version, tenant_id, factory_id, icon, tags")
        .single();
      setSaving(false);
      if (error) { setFormError(error.message); return; }
      setAgents((prev) => [...prev, data as unknown as AgentRow]);
    }
    // If saving a super-specialist, update the parent's sub_specialists list
    const parentSlug = newForm.level === "super-specialist" ? newForm.parent_slug : null;
    if (parentSlug) {
      const parent = agents.find((a) => a.slug === parentSlug);
      if (parent) {
        const allSubs = agents
          .filter((a) => a.parent_slug === parentSlug && a.level === "super-specialist" && a.slug !== newForm.slug)
          .map((a) => ({ slug: a.slug, name: a.name }));
        allSubs.push({ slug: newForm.slug, name: newForm.name });
        const parentMeta = { ...(parent.metadata ?? {}), sub_specialists: allSubs };
        await supabase.from("agent_definitions").update({ metadata: parentMeta }).eq("id", parent.id);
      }
    }
  ...
  // Squads derived from filtered agents (respects tenant/factory scope)
  ...
  async function cloneAgent(agent: AgentRow) {
    ...
    const { error } = await supabase.from("agent_definitions").insert({
      ...
      metadata: agent.metadata, spec: agent.spec, icon: agent.icon,
      origin: "user", enabled: true, tenant_id: tenantId, factory_id: factoryId,
      origin_id: null, version: agent.version,
    }).select("id, slug, name, level, enabled, origin, parent_slug, metadata, squad, spec, version, tenant_id, factory_id, icon, tags").single();
    if (error) { alert(`Clone failed: ${error.message}`); return; }
    // Reload agents (scoped to tenant)
  ...
  function AgentCard({ agent, color }: { agent: AgentRow; color: string }) {
    const icon = (agent.metadata as Record<string, unknown>)?.icon as string | undefined;
    return (
      <div style={{ background: "var(--surface0)", border: "1px solid var(--surface1)", borderRadius: 10, padding: "12px 14px", opacity: agent.enabled ? 1 : 0.55, transition: "opacity 0.15s" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 3, flexShrink: 0 }}>
            <button onClick={() => openEditModal(agent)} title="Edit" style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--overlay0)", padding: 2, fontSize: 11 }}>✏️</button>
            <button onClick={() => cloneAgent(agent)} title="Clone" style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--overlay0)", padding: 2 }}><Copy size={11} /></button>
        ...
        <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", padding: "2px 6px", borderRadius: 4, background: ((agent.spec as Record<string, unknown>)?.autonomy as string ?? "auto") === "auto" ? "var(--green)20" : "var(--yellow)20", color: ((agent.spec as Record<string, unknown>)?.autonomy as string ?? "auto") === "auto" ? "var(--green)" : "var(--yellow)" }}>
            {((agent.spec as Record<string, unknown>)?.autonomy as string ?? "auto").toUpperCase()}
          </span>
          <span style={{ fontSize: 10, color: "var(--overlay0)" }}>
            {(agent.spec as Record<string, unknown>)?.autonomy === "human" ? "🛡 human" : "auto"}
          </span>
        </div>
      </div>
    );
      ...
      {showNew && (
        <div style={{ flex: 1, overflowY: "auto", background: "var(--mantle)" }}>
          <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--surface0)", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "var(--mantle)", zIndex: 10 }}>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{editAgent ? `Edit: ${editAgent.name}` : "New Agent"}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr 1fr", gap: 12, alignItems: "end" }}>
            ...
            <FormField label="Icon">
              <div style={{ position: "relative" }}>
                <button
                  ...
                  style={{ width: 40, height: 36, borderRadius: 8, border: "1px solid var(--surface1)", background: "var(--surface0)", cursor: "pointer", fontSize: 20, display: "flex", alignItems: "center", justifyContent: "center" }}
                >
                  {newForm.icon || "🤖"}
                </button>
                {showIconPicker && (
            ...
            <FormField label="Name">
              <input
                ...
                onChange={(e) => {
                  ...
                  setNewForm((f) => ({ ...f, name, slug: editAgent ? f.slug : name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") }));
                }}
            ...
            <FormField label="Slug">
              <input value={newForm.slug} onChange={(e) => setNewForm((f) => ({ ...f, slug: e.target.value.toLowerCase().replace(/\s+/g, "-") }))} placeholder="my-agent" style={{ ...inputStyle, fontFamily: "var(--font-mono)" }} disabled={!!editAgent} />
            </FormField>
          </div>
          <FormField label="Squad (optional)">
            <select value={newForm.squad} onChange={(e) => setNewForm((f) => ({ ...f, squad: e.target.value }))} style={inputStyle}>
              <option value="">— No squad —</option>
              {[...new Set(agents.map((a) => a.squad).filter(Boolean))].sort().map((s) => <option key={s!} value={s!}>{s}</option>)}
            </select>
          </FormField>
          {/* ── Specialization ── */}
          <div style={{ padding: "12px 14px", borderRadius: 10, border: "1px solid var(--surface0)", background: "var(--base)" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--subtext0)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Specialization</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <FormField label="Level">
                <select value={newForm.level} onChange={(e) => setNewForm((f) => ({ ...f, level: e.target.value as AgentLevel | "", parent_slug: "" }))} style={inputStyle}>
                  <option value="">Generic</option>
                  <option value="specialist">Specialist</option>
                  <option value="super-specialist">Super-specialist</option>
                </select>
              </FormField>
              {newForm.level === "super-specialist" && (
                <FormField label="Parent Specialist *">
                  <select value={newForm.parent_slug} onChange={(e) => setNewForm((f) => ({ ...f, parent_slug: e.target.value }))} style={inputStyle}>
                    <option value="">— select parent —</option>
                    {agents
                      .filter((a) => (a.level === "specialist" || !a.level) && a.slug !== newForm.slug)
                      .sort((a, b) => a.name.localeCompare(b.name))
                      .map((a) => <option key={a.slug} value={a.slug}>{a.name} ({a.slug})</option>)}
                  </select>
                </FormField>
              )}
            </div>
            {newForm.level === "super-specialist" && (
              <div style={{ fontSize: 11, color: "var(--overlay0)", marginTop: 6, lineHeight: 1.5 }}>
                Super-specialists are nested under their parent. The parent automatically knows about its sub-specialists and can delegate tasks to them.
              </div>
            )}
            {/* Show sub-specialists for specialists */}
            {(newForm.level === "specialist" || !newForm.level) && newForm.slug && (() => {
              const subs = agents.filter((a) => a.parent_slug === newForm.slug && a.level === "super-specialist");
              if (subs.length === 0) return null;
              return (
                <div style={{ marginTop: 10, padding: "8px 12px", borderRadius: 8, background: "var(--surface0)", fontSize: 12 }}>
                  <div style={{ fontWeight: 600, color: "var(--subtext0)", marginBottom: 4 }}>Sub-specialists ({subs.length})</div>
                  {subs.map((s) => (
                    <div key={s.slug} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0", color: "var(--text)" }}>
                      <Bot size={12} style={{ color: "var(--blue)", flexShrink: 0 }} />
                      <span>{s.name}</span>
          ...
          {/* ── Runtime & Autonomy ── */}
          <div style={{ padding: "12px 14px", borderRadius: 10, border: "1px solid var(--surface0)", background: "var(--base)" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "var(--subtext0)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Runtime &amp; Autonomy</div>
            <FormField label="Autonomy">
              <select value={newForm.autonomy} onChange={(e) => setNewForm((f) => ({ ...f, autonomy: e.target.value as AgentAutonomy }))} style={inputStyle}>
                <option value="auto">auto — runs without approval gates</option>
                <option value="human">human — requires human sign-off</option>
              </select>
            </FormField>
          </div>

          {/* ── Description (persona) ── */}
          <FormField label="Description — who this agent is, what it does, how it behaves">
            <textarea
              value={newForm.instructions}
              onChange={(e) => setNewForm((f) => ({ ...f, instructions: e.target.value }))}
              placeholder={`Describe the agent's role and responsibilities.\n\nExample: "Creates Architecture Decision Records (ADRs) for significant decisions. Defines tech stack with pinned versions. Designs data model with RLS policies."`}
              rows={6}
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => setSelectedTools(availableTools.map((t) => t.slug))} style={{ fontSize: 10, fontWeight: 600, color: "var(--blue)", background: "none", border: "none", cursor: "pointer", fontFamily: "var(--font-sans)", padding: 0 }}>Select all</button>
                  <button onClick={() => setSelectedTools([])} style={{ fontSize: 10, fontWeight: 600, color: "var(--overlay0)", background: "none", border: "none", cursor: "pointer", fontFamily: "var(--font-sans)", padding: 0 }}>Unselect all</button>
                </div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4, padding: "8px 10px", background: "var(--surface0)", borderRadius: 8, border: "1px solid var(--surface1)" }}>
                {availableTools.map((t) => {
                  ...
                  return (
                    <button
                      key={t.slug}
                      onClick={() => setSelectedTools((prev) => checked ? prev.filter((s) => s !== t.slug) : [...prev, t.slug])}
                      style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "5px 4px", background: "none", border: "none", cursor: "pointer", textAlign: "left", borderRadius: 5, color: "inherit" }}
                    >
                      {checked
...
type Tab = "agents" | "pipelines" | "projects";
    ...
    <div style={{ display: "flex", flex: 1, height: "100vh", overflow: "hidden", fontFamily: "var(--font-sans)", color: "var(--text)", background: "var(--base)" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ height: 50, borderBottom: "1px solid var(--surface0)", background: "var(--mantle)", display: "flex", alignItems: "center", padding: "0 20px", gap: 4, flexShrink: 0 }}>
          ...
          {/* Projects — inline tab between Pipelines and SIPOC */}
          <button
            onClick={() => setTab("projects")}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 8, border: "none", background: tab === "projects" ? "var(--surface0)" : "transparent", color: tab === "projects" ? "var(--text)" : "var(--overlay0)", fontSize: 13, fontWeight: tab === "projects" ? 700 : 500, cursor: "pointer", fontFamily: "var(--font-sans)", transition: "all 0.15s" }}
          >
        ...
        {authReady && (
          <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
            {tab === "agents"    && <AgentsTab userRole={userRole} hideBuiltIn={hideBuiltIn} tenantId={tenantId} factoryName={ctxFactoryName} factoryOrigin={factoryOrigin} factoryId={factoryId} inheritedFactoryIds={inheritedFactoryIds} />}
            {tab === "pipelines" && <PipelinesTab tenantId={tenantId} plan={plan} hideBuiltIn={hideBuiltIn} factoryName={ctxFactoryName} factoryOrigin={factoryOrigin} factoryId={factoryId} inheritedFactoryIds={inheritedFactoryIds} />}
            {tab === "projects"  && (
      ...
      {showWizard && wizardEnabled && factoryId && (
        <WizardPanel factoryId={factoryId} monthlyLimit={wizardLimit} onClose={() => setShowWizard(false)} />
      )}
    </div>
  );
}
...
function PipelineRow({ pipeline, selected, onClick, onEdit, onDelete }: { pipeline: Pipeline; selected: boolean; onClick: () => void; onEdit?: () => void; onDelete?: () => void }) {
  ...
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 8px", borderRadius: 8, marginBottom: 2, background: selected ? "var(--surface0)" : "transparent", borderLeft: selected ? "3px solid var(--blue)" : "3px solid transparent", cursor: "pointer" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{pipeline.name}</div>
      ...
      {onDelete && (
        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} title="Delete pipeline" style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--overlay0)", padding: 2, flexShrink: 0 }}>
          <Trash2 size={11} />
        </button>
      )}
    </div>
