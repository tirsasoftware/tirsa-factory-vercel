...
import {
  ...
  Loader2, RefreshCw, Eye, EyeOff, Save, Zap, X, Shield, Terminal, Trash2,
} from "lucide-react";
...
interface CliAgentCardProps {
  agent:             CliAgentMeta;
  configured:        Set<string>;           // provider keys "serviceId:VAR"
...
function CliAgentCard({ agent, configured, cliConfigured, tenantId, session, onCliSaved }: CliAgentCardProps) {
  ...
  // Determine if each var is covered by a provider key or a CLI key
      ...
      {open && (
        <div style={{ borderTop: "1px solid var(--surface0)", padding: "16px" }}>
          ...
          {agent.vars.map((v) => {
            ...
            const providerName = PROVIDERS.find((p) => v.providerIds.includes(p.id))?.name ?? v.providerIds[0];
              ...
              <div key={v.varName} style={{ marginBottom: 14, padding: "12px 14px", background: "var(--surface0)", borderRadius: 9 }}>
                ...
                {providerHas && !cliHas && (
                  <div style={{ fontSize: 11, color: "var(--subtext0)", marginBottom: 8 }}>
                    Using key from <strong>{providerName}</strong> provider above.
                  </div>
                )}

                {/* Toggle for CLI-specific key */}
                {providerHas && !cliHas && (
                ...
                {/* Show input if no provider key, or user chose CLI-specific */}
...
export default function ProvidersPage() {
  ...
  /* Load CLI keys once tenantId + session are known */
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", padding: "32px 24px 80px" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
            <div>
              ...
              <p style={{ color: "var(--subtext0)", fontSize: 13, maxWidth: 520 }}>
                Configure API keys for each provider directly here. Keys are stored encrypted — never exposed to the browser after saving.
              </p>
            </div>
            <button
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
              ...
              <div>
                ...
                <p style={{ color: "var(--subtext0)", fontSize: 12, margin: 0, marginTop: 2 }}>
                  Configure API keys for headless CLI agents. You can reuse keys from the providers above or set CLI-specific keys.
                </p>
              </div>
            </div>
