...
export async function GET(req: NextRequest) {
  ...
  if (action === "approve") {
    // Check project is actually waiting
    if (project?.status !== "waiting") {
      return htmlResponse("Not Waiting", `Project "${project?.name ?? "?"}" is not waiting for approval (status: ${project?.status}).`, "warning");
    }

    // Approve: update project + sprint status, insert approval event
    await s.from("projects").update({ status: "executing" }).eq("id", gate.project_id);
    ...
    // Reject
    await s.from("projects").update({ status: "paused" }).eq("id", gate.project_id);
    await s.from("sprints")
