...
async function assertCanEdit(
  ...
  const { data } = await sb
    ...
    .eq("user_id", userId)
    .single();
  if (!data || !["owner", "admin"].includes(data.role as string)) throw new Error("Forbidden");
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = await req.json() as { name?: string; description?: string; steps?: unknown[]; is_active?: boolean; mode?: string };
