...
async function getSuggestedVars(tenantId: string): Promise<Record<string, string>> {
  ...
  if (storageRow?.secret_value) {
    try {
      ...
      if (cfg.type === "supabase" && cfg.key) suggested.SUPABASE_SERVICE_ROLE_KEY = cfg.key;
    } catch { /* ignore */ }
  }

  // Fallback to command-center's own env vars
  ...
  if (!suggested.SUPABASE_SERVICE_ROLE_KEY && process.env.SUPABASE_SERVICE_ROLE_KEY) {
    suggested.SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
  }
