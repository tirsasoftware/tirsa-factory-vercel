...
// "provisioning" is included so a stuck trigger attempt can be retried from the UI
// "completed" is included so a specific step (e.g. sprint-push commit) can be re-run
const RESUMABLE_STATUSES = ["paused", "waiting", "queued", "ready", "provisioning", "completed"];
...
export async function POST(
  ...
  try {
    ...
    // "provisioning"/"running"/"executing" would be rejected by the slot
    // function as project-busy, so flip them down to "paused" first. The
    // slot is re-acquired atomically by dispatchSprint.
    if (["provisioning", "running", "executing"].includes(project.status as string)) {
      await sb.from("projects").update({ status: "paused" }).eq("id", projectId);
    }
