...
export async function GET(
  ...
  try {
    ...
    // ── Storage files — try local first, then Supabase ───────
    ...
    if (localBase && tenantSlug && factorySlug) {
      ...
      if (isWithinBase(resolve(targetDir), localBase) && existsSync(targetDir)) {
        storageFiles = listLocal(targetDir, "", 6, true).map((f) => ({ path: f.path, size: f.size }));
        storageBackend = "local";
      } else {
        const projRoot = localProjectRoot(localBase, tenantSlug, factorySlug, projectSlug);
        if (isWithinBase(resolve(projRoot), localBase) && existsSync(projRoot)) {
          storageFiles = listLocal(projRoot).map((f) => ({ path: f.path, size: f.size }));
          storageBackend = "local";
        }
      }
    }

    // If local didn't find anything, try Supabase
    if (storageFiles.length === 0) {
      const basePrefix = (tenantSlug && factorySlug) ? sprintPath(tenantSlug, factorySlug, projectSlug, sprintNum) : `${projectSlug}/sprint-${sprintNum}`;
      const prefix = subdirParam
        ? `${basePrefix}/${subdirParam}`
        : basePrefix;
      try {
        const sbFiles = await listStoragePrefix(sb, TP_BUCKET, prefix);
        if (sbFiles.length > 0) {
          storageFiles = sbFiles;
          storageBackend = "supabase";
        }
      } catch { /* no storage */ }
    }

    if (storageFiles.length === 0 && localBase && tenantSlug && factorySlug) {
      localError = `No files found locally at staging/sprint-${sprintNum} or in Supabase bucket.`;
    }
