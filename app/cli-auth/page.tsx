...
function CliAuthInner() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (!session) { router.replace(`/login?next=/cli-auth?state=${state}&port=${port}`); return; }
          ...
          <>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <select
                ...
                style={selectStyle}
                disabled={(selectedTenant?.factories.length ?? 0) <= 1}
              >
...
const centeredPage: React.CSSProperties = {
  ...
  minHeight: "100vh", background: "var(--base)", fontFamily: "var(--font-sans)", color: "var(--text)",
};
